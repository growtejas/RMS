    ...
    const fetchResources = async () => {
      try {
        ...
        const [employeesRes, skillsRes] = await Promise.all([
          cachedApiGet<{ emp_id: string; full_name: string }[]>(
            "/employees/employees",
            { signal: controller.signal },
          ),
          cachedApiGet<{ skill_id: number; skill_name: string }[]>(
            "/skills/",
            { signal: controller.signal },
          ),
        ]);
        ...
        const resourceList = await Promise.all(
          employees.map(async (emp) => {
            const [skillsRows, availabilityRows] = await Promise.all([
              cachedApiGet<
                {
                  skill_id: number;
              ...
              >(`/employees/${emp.emp_id}/availability`, {
                signal: controller.signal,
              }).catch(() => [] as { availability_pct: number; effective_from: string }[]),
            ]);
            ...
            const availabilityHistory = availabilityRows ?? [];
