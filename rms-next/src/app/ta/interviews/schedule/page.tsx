...
function ScheduleInterviewPageInner() {
  ...
  const requisitionItemId = requisitionItemIdRaw ? Number.parseInt(requisitionItemIdRaw, 10) : NaN;
  ...
  const nextRoundNumber = useMemo(() => {
    ...
    const max = rows
      .filter(
        (iv) =>
          iv.candidate_id === candidateId && (iv.requisition_item_id ?? null) === requisitionItemId,
      )
      ...
      <div className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
        ...
        <div className="mt-4">
          <button
            ...
            onClick={() => router.push("/ta/interviews")}
          >
            Back to interviews
          </button>
        </div>
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <InterviewScheduleForm
        ...
        onScheduled={() => {
          router.push("/ta/interviews");
        }}
        onCancel={() => router.push("/ta/interviews")}
        submitMode="default"
      />
    </div>
  );
...
export default function TaScheduleInterviewPage() {
  return (
