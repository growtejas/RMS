...
import { rejectBody } from "@/lib/validators/workflow";
...
export async function POST(req: Request, { params }: Ctx) {
  try {
    ...
    const parsed = await parseFastapiJsonBody(req, rejectBody);
    if (!parsed.ok) {
    ...
    const row = await db.transaction(async (tx) => {
      ...
      return RequisitionWorkflowEngine.reject(dbx, {
        reqId,
        userId: user.userId,
    ...
    return workflowCatch(e, "[POST workflow/reject]");
  }
}
