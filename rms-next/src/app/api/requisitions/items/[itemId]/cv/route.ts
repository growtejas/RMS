...
import { deleteItemCv, uploadItemCv } from "@/lib/services/requisitions-write-service";
...
/** GET /api/requisitions/items/{itemId}/cv */
export async function GET(req: Request, { params }: Ctx) {
  try {
    ...
    const key = item.cvFileKey;
    if (!key) {
      return NextResponse.json(
        { detail: "CV file not available for this item" },
        { status: 404 },
      );
    }
    ...
    try {
      ...
      return new NextResponse(buf, {
        headers: {
          ...
          "Content-Disposition": `inline; filename="requisition_item_${itemId}_cv.pdf"`,
        },
      });
    } catch {
      return NextResponse.json(
    ...
    return referenceWriteCatch(e, "[GET /api/requisitions/items/[itemId]/cv]");
  }
}

/** POST /api/requisitions/items/{itemId}/cv */
export async function POST(req: Request, { params }: Ctx) {
  try {
    ...
    if (!(file instanceof File) || file.size === 0) {
      return NextResponse.json({ detail: "cv_file is required" }, { status: 400 });
    }
    const body = await uploadItemCv(
      itemId,
      user.organizationId,
      {
    ...
    return referenceWriteCatch(e, "[POST /api/requisitions/items/[itemId]/cv]");
  }
}

/** DELETE /api/requisitions/items/{itemId}/cv */
export async function DELETE(req: Request, { params }: Ctx) {
  try {
    ...
    const body = await deleteItemCv(itemId, user.organizationId, user.userId);
    return NextResponse.json(body);
  } catch (e) {
    return referenceWriteCatch(e, "[DELETE /api/requisitions/items/[itemId]/cv]");
  }
}
