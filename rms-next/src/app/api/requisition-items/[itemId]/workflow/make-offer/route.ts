...
import { makeOfferBody } from "@/lib/validators/workflow";
...
export async function POST(req: Request, { params }: Ctx) {
  try {
    ...
    const parsed = await parseFastapiJsonBody(req, makeOfferBody);
    if (!parsed.ok) {
    ...
    const updated = await db.transaction(async (tx) => {
      ...
      return RequisitionItemWorkflowEngine.makeOffer(dbx, {
        itemId,
        userId: user.userId,
        userRoles: user.roles,
        candidateId: parsed.data.candidate_id ?? undefined,
        offerDetails: (parsed.data.offer_details ?? undefined) as
    ...
    return workflowCatch(e, "[POST item workflow/make-offer]");
  }
}
