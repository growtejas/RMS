...
import { fulfillBody } from "@/lib/validators/workflow";
...
export async function POST(req: Request, { params }: Ctx) {
  try {
    ...
    const parsed = await parseFastapiJsonBody(req, fulfillBody);
    if (!parsed.ok) {
    ...
    const updated = await db.transaction(async (tx) => {
      ...
      return RequisitionItemWorkflowEngine.fulfill(dbx, {
        itemId,
        userId: user.userId,
    ...
    return workflowCatch(e, "[POST item workflow/fulfill]");
  }
}
