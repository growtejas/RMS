...
import { swapTaBody } from "@/lib/validators/workflow";
...
export async function POST(req: Request, { params }: Ctx) {
  try {
    ...
    const parsed = await parseFastapiJsonBody(req, swapTaBody);
    if (!parsed.ok) {
    ...
    const updated = await db.transaction(async (tx) => {
      ...
      return RequisitionItemWorkflowEngine.swapTa(dbx, {
        itemId,
    ...
    return workflowCatch(e, "[POST item workflow/swap-ta]");
  }
}
